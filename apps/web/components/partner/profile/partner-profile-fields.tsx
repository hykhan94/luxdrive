...
// ============================================
// apps/web/components/profile/partner-profile-fields.tsx
// The Company + Bank field sections of the partner profile, wired to
...
import { PARTNER_FIELD_BY_KEY } from "@/lib/partner-profile-fields";
import { useRejectionProgress } from "@/components/partner/profile/rejection-progress-context";
import { SavingIndicator } from "@/components/partner/profile/saving-indicator";
...
export interface PartnerProfileFieldsProps {
  profileId: string;
  status: string;
  editable: boolean;
  logoUrl: string | null;
...
export function PartnerProfileFields({
  ...
  const logoUpload = useDocumentUpload<string | null>({
    ...
    record: async ({ filePath }) => {
      const res = await partnerApi.uploadLogo({ logoUrl: filePath });
  ...
  const renderField = (key: string) => {
    const cfg = PARTNER_FIELD_BY_KEY[key];
    if (!cfg) return null;
    const item = byKey[key];
    const flagged = !!item;
    const isPartnerRequest = item?.source === "partner_request";
    ...
    //   partner submits (status flips to PENDING_REVIEW), `editable` goes false
    //   and the outer !editable branch re-locks. No mid-flow re-lock — partner
    //   should be free to type, correct typos, or paste different values right
    //   up until they hit Submit.
    // - CHANGES_REQUESTED informational (partner request / admin note): same
              ...
              {flagged &&
                (isInformational ? (
                  <span className="rounded border border-sky-500/40 bg-sky-500/20 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-sky-400">
                    {isPartnerRequest ? "Editable" : "Note"}
                  </span>
                ) : (
  ...
  return (
    <>
      {/* One-shot attention pulse for flagged fields/docs. Plays a 2s
          ...
          admin rejection, sky for informational partner-request / note). */}
      <style jsx global>{`
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="space-y-1">
            ...
            {logoBusy ? (
              ...
              // APPROVED (the general `editable` prop). Any active partner
