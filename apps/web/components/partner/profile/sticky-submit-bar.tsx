...
// ============================================
// apps/web/components/profile/sticky-submit-bar.tsx
...
export interface StickySubmitBarProps {
  ...
  /** Mode-1 onboarding section chips (mutually exclusive with `rejection` / `partnerRequest`). */
  sections?: ProgressSection[] | null;
  /** Mode-2/4 flagged-item progress (mutually exclusive with `sections` / `partnerRequest`). */
  rejection?: {
    addressed: number;
    total: number;
  ...
  /**
   * Partner-request cycle progress (mutually exclusive with `sections` /
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 sm:px-5">
        <div className="min-w-0 flex-1">
          {rejection ? (
            ...
            <PartnerRequestCount {...partnerRequest} />
          ) : sections ? (
...
/**
 * Partner-request-cycle progress. Same visual footprint as RejectionCount,
 * but the "left" count is informational — a partner who only wanted to edit
