...
// ============================================
// apps/web/components/profile/rejection-progress-context.tsx
...
// client, how many flagged items the partner has addressed — updating live as
...
/**
 * Distinguishes the three flows that all use PartnerReviewComment. Driven
...
export interface RejectionProgressValue {
   ...
   * items only. The sticky bar / banner use these because a partner-request
  ...
  /**
   * Partner-request scope: total granted-edit items, and how many the partner
   * has actually modified (current value ≠ snapshot). The submit bar uses
   * `partnerRequestChangedCount > 0` to gate submission during a pure
   * partner-request cycle — a partner who hasn't touched anything yet
   * shouldn't see the "ready to submit" state.
   */
  partnerRequestTotal: number;
  partnerRequestChangedCount: number;
}
...
const EMPTY: RejectionProgressValue = {
  ...
  partnerRequestTotal: 0,
  partnerRequestChangedCount: 0,
};
...
export function RejectionProgressProvider({
  ...
  const computed = React.useMemo(() => {
    ...
    const items: RejectionFlaggedItem[] = keys.map((key) => {
      ...
      } else if (comment.startsWith("Change requested by partner:")) {
        source = "partner_request";
      } else {
        source = "admin_rejection";
      }
    ...
    // Partner-request "changed" — has the partner actually modified any of
    ...
    // object because partner-request items intentionally never show "addressed"
    // UI — this is only for the submit-gate check.
    const partnerRequestItems = items.filter(
      (i) => i.source === "partner_request",
    );
    const partnerRequestTotal = partnerRequestItems.length;
    const partnerRequestChangedCount = partnerRequestItems.filter((it) => {
