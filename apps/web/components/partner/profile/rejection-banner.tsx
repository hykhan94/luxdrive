...
// ============================================
// apps/web/components/profile/rejection-banner.tsx
...
export function RejectionBanner({ className }: RejectionBannerProps) {
  ...
  // No admin rejections but partner has open partner_request / admin_comment
  // items. Show a friendly sky-blue banner explaining editing is enabled.
  if (!rejectionActive) {
    const partnerRequestCount = items.filter(
          ...
          <div className="text-sm">
            ...
            <p className="mt-0.5 text-sky-400/80">
              {partnerRequestCount > 0 && (
                <>
                  {partnerRequestCount} field
                  {partnerRequestCount === 1 ? "" : "s"} you asked to update{" "}
                  {partnerRequestCount === 1 ? "is" : "are"} unlocked below.
                </>
              )}
              {partnerRequestCount > 0 && adminNoteCount > 0 && " "}
              {adminNoteCount > 0 && (
                <>
