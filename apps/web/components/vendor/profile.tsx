// TODO(shared-profile): Duplicated from components/partner/profile-panel.tsx
// Portal-specific bits: partnerApi → vendorApi, PARTNER_PROFILE_FIELDS →
// VENDOR_PROFILE_FIELDS, team-management endpoints stripped (vendor's admin
// creates team members centrally; vendor sees them read-only).
...
// Vendor Portal — Company Profile & Team
...
import { VENDOR_PROFILE_FIELDS } from "@/lib/vendor-profile-fields";
import { useFieldAutosave } from "@/hooks/use-field-autosave";
import { RejectionProgressProvider } from "@/components/vendor/profile/rejection-progress-context";
import { UnsavedChangesGuard } from "@/components/vendor/profile/unsaved-changes-guard";
import { VendorProfileFields } from "@/components/vendor/profile/vendor-profile-fields";
import { VendorProfileDocuments } from "@/components/vendor/profile/vendor-profile-documents";
import { VendorSubmitBar } from "@/components/vendor/profile/vendor-submit-bar";
import { RejectionBanner } from "@/components/vendor/profile/rejection-banner";
...
interface CompanyProfile {
  ...
  // the pre-rejection baseline so we can flag fields the vendor
  ...
  // Non-null when vendor is in a "requested edit approved by admin"
  // cycle. Used by the frontend to know which fields admin granted
  // permission to edit and drive the sticky-bar progress ("N of M
  // edited") for the vendor-request flow.
...
export default function VendorProfilePanel({
  ...
  // Team management is still "Coming Soon" — the team section shows a
  // placeholder card. When we actually build vendor-side team management,
  // reintroduce teamMembers state + fetchTeam here and mirror the sibling partner file's
  // patterns.
  ...
  const saveField = useCallback(
    async (group: string, body: Record<string, unknown>) => {
      const res =
        group === "bank"
          ? await vendorApi.updateBankDetails(body)
          : await vendorApi.updateCompanyInfo(body);
  ...
  // ---- Team fetch omitted while team section is Coming Soon ----

  const fetchChangeRequests = useCallback(async () => {
    if (profile?.status !== "APPROVED") return;
    try {
      const res = await vendorApi.getProfileChangeRequests();
      if (res.data) {
  ...
  useEffect(() => {
    if (
      ...
      vendorApi
        .markAllNotificationsAsRead("PROFILE")
        .then(() => refreshBadges())
        .catch(() => {});
    }
  ...
  const handleSubmitChangeRequest = async () => {
    ...
    try {
      await vendorApi.requestProfileChanges({
  ...
  // If ALL of the vendor's outstanding review comments are from an approved
  ...
  const statusInfo =
    profile.status === "CHANGES_REQUESTED" && allCommentsAreVendorRequests
      ? {
  ...
  const requiredFieldsFilled = VENDOR_PROFILE_FIELDS.filter(
    (f) => f.required,
  ).every((f) => String(autosave.values[f.key] ?? "").trim() !== "");
  const onboardingComplete =
    requiredFieldsFilled && allDocsUploaded && mouComplete;

  // Bar visibility: visible whenever the vendor has something to submit —
  ...
  const handleSubmit = async () => {
    setSubmitting(true);
    try {
      await vendorApi.submitProfileForReview();
  ...
  // Team handlers intentionally omitted — vendor doesn't manage team from
  // this panel. Add/remove/deactivate flow through admin. If we ever add
  // vendor-side management, the corresponding vendorApi methods need to
  // be added first.
  ...
  // a top-of-page banner so the vendor sees the issue without scrolling. The
  ...
  // expiring-soon docs are surfaced. When the vendor clicks Request
  ...
  // PENDING_REVIEW / APPROVED / SUSPENDED there's nothing the vendor can act
    ...
    >
      <div className="relative space-y-6 w-full pb-32">
        {/* Status Banner — CHANGES_REQUESTED / rejection state is now driven by
          RejectionBanner (rendered from VendorProfileFields' provider
        ...
        {/* Matches the vendor portal pattern: stacked individual banners, one
          ...
          Vendor profile has 2 buckets (no "incomplete uploads" concept
        ...
        {/* Expiry banners — APPROVED-only. Pre-approval the warning is
          ...
          vendor can't trigger the renewal flow (Request Changes is
        ...
        {/* ===== TEAM MEMBERS =====
          ...
          built on the vendor side (invite, role-change, deactivate
          actions all exist), but the rollout isn't finalised, and the
          live list was confusing vendors who don't have real
        ...
        <div className="p-6 bg-neutral-900 border border-neutral-800 rounded-xl">
          ...
          <div className="border border-dashed border-neutral-700 rounded-xl py-12 px-6 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-sm text-gray-500 max-w-md">
              ...
              tracking so you can onboard colleagues to your vendor account.
                  ...
                  <div className="space-y-2 max-h-[40vh] overflow-y-auto pr-1">
                    {[
                      ...
                      },
                      {
                        ...
                        icon: ScrollText,
                        fields: [
                          // MOU expiry is derived from whichever PDF the vendor
                          // uploads — you can't change one without the other.
                          // We expose only the document; if it's replaced, the
                          // vendor picks a new expiry as part of that upload.
